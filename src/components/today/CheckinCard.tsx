 ...
 * --brand-accent so its small uppercase label clears 4.5:1 on the red), an
 * optional leading step number, the big day title, a one-line description, a
 * solid red CTA, an optional "N of 6 answered" progress line, and -- on desktop
 * only -- a photograph. `bg-background` resolves to ink inside the Today
 ...
 * language regardless of the time of day. `step` is the card's fixed daily slot
 * (Morning = 1, midday check-in = 2, Night = 3) -- omitted for the weekend hero,
 * which isn't one of the three numbered routines.
 */
    ...
    <article className="border-2 border-brand-accent bg-background">
      ...
      <div className="flex flex-col gap-5 p-5 sm:flex-row sm:items-stretch">
        <div className="flex flex-1 gap-4">
          {typeof step === "number" && <StepBadge n={step} />}
